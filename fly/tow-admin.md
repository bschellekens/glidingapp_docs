...
GlidingApp represents each aerotow as **two separate flight records** connected via the `uuid` field. The glider flight has start_methode="tow", and the tug flight has start_methode="tmg-a" (TMG) or "sep-a" (SEP).
...
:::info Migrating to a Two-Flight Model
...
4. System saves changes immediately and sync changes to the launch point