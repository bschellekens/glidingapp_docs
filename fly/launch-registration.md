...
Note: Please do keep in mind that the administration around a proficiency check is often deemed so large that it makes more sense to fly the hours/launches under supervision of an instructor. 
...
When a day is closed, the system assigns training flight flags based on instructor notes, calculates motor glider block time, and sends individual email summaries to every pilot who flew.

The grace period before locking is configurable in club settings. After locking, non-admin users with privacy mode enabled cannot view other pilots' flights from that day.
...
### Setup launch tower password

**What:** **Launch tower access** uses a temporary 4-digit password that expires after first use or 10 minutes. The launch tower account allows flight logging without personal login credentials.
**When:** Start of flying day or when launch marshal needs access  
**Permission:** "Edit Flights"

**How:**
1. Go to **Flights** → **Flight Admin** → **Select day**
2. Click **Launch Tower Password** button
3. System generates 4-digit code valid for 10 minutes
4. Share code with launch marshal
5. Launch marshal logs in with username "startwagen" and temporary password
6. Password expires after first use or 10 minutes

Note, by default the launch tower user remains logged in the computer/tablet used at the launch point.


### Configure OGN mode and default categories

**What:** Set automatic timekeeping method and pre-selected flight categories  
**When:** Start of flying day, when OGN receiver status changes, or when changing typical flight types  
**Permission:** "Edit Flights"
...
**When:** End of flying session (half-day, full-day, or evening)  
**Permission:** "Edit Flights"
...
2. Check Daily Operations Report, Tug Log, and Winch Log for completeness
3. Click **Close Day** button
4. System automatically:
   - Assigns training flights based on instructor notes
   - Calculates motor glider block time
   - Sends email to each pilot with their flights summary
   - Includes link to daily report and start admin
5. Day status changes to "closed", further edits require admin permission